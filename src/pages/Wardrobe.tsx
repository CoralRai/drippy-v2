...
function CategoryIcon({ category, className = "h-4 w-4" }: { category: string; className?: string }) {
  switch (category) {
    case "top": return <Shirt className={className} />;
    case "bottom": return <Shirt className={`${className} rotate-180`} />;
    case "footwear": return <Footprints className={className} />;
    case "outerwear": return <CloudSun className={className} />;
              ...
              .map((group) => (
                <div key={group.value}>
                  <h3 className="font-display font-bold text-lg mb-3 flex items-center gap-2">
                    <CategoryIcon category={group.value} className="h-5 w-5 text-primary" />
                    {group.label}s
