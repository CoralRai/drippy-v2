...
const SYSTEM_PROMPT = `You are a style advisor for Drippy. Give practical, specific fashion advice: color theory, body proportions, occasion-appropriateness, current trends. Keep answers to 2-4 sentences. No emojis, no hype — clear and direct.`;
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      const greeting = currentOutfit?.top
        ? `Style advisor here. You're looking at: ${currentOutfit.top} + ${currentOutfit.bottom}. Color harmony is ${currentOutfit.colorHarmony}% — ask me anything.`
        : `Style advisor here. You're browsing ${occasion} looks. Ask me anything — color advice, what shoes to pick, how to dress for your body type.`;
        ...
        <div className="fixed bottom-6 right-6 z-50 w-80 sm:w-96 flex flex-col rounded-2xl border border-border bg-card shadow-2xl overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-primary text-primary-foreground">
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4" />
              <span className="font-semibold text-sm">Style Advisor</span>
            </div>
