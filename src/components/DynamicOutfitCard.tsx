...
export interface GeneratedOutfit {
  ...
  outfit_json: {
    ...
    styling_tip: string;
    body_type_reason: string;
    skin_tone_reason: string;
    uses_owned_item: boolean;
    style_tags: string[];
...
// Normalizes any gender value from the quiz into a search-friendly term.
// Handles common variants (Male/Man/M, Female/Woman/F) case-insensitively.
// Returns "" for unisex/unknown so the query stays neutral.
function getGenderSearchTerm(gender?: string): string {
  const g = (gender ?? "").trim().toLowerCase();
  if (["male", "man", "men", "m", "boy"].includes(g)) return "mens";
  if (["female", "woman", "women", "f", "girl"].includes(g)) return "womens";
  return ""; // unisex / not specified
}

// Module-level in-memory cache keyed by `${genderTerm}-${color}-${name}`.
// Using the normalised term (not the raw gender) so "Male"/"male"/"M" all hit the same entry.
const imageCache = new Map<string, string>();

function getDetFallbackUrl(item: OutfitItem, gender?: string): string {
  const genderTerm = getGenderSearchTerm(gender);
  const keywords = [item.color, item.name, genderTerm, "clothing", "apparel"]
    .filter(Boolean)
    .join(",");
  return `https://source.unsplash.com/400x520/?${encodeURIComponent(keywords)}`;
}

async function fetchOutfitItemImage(item: OutfitItem, gender?: string): Promise<string> {
  const genderTerm = getGenderSearchTerm(gender);
  // Cache key uses the normalised gender term so variant spellings share the same entry
  const cacheKey = `${genderTerm || "unisex"}-${item.color}-${item.name}`;
  const cached = imageCache.get(cacheKey);
  if (cached) return cached;

  const query = [item.color, item.name, genderTerm, "fashion clothing"].filter(Boolean).join(" ");
  const fallback = getDetFallbackUrl(item, gender);
  ...
  if (unsplashKey) {
    try {
      const res = await fetch(
        `https://api.unsplash.com/search/photos?query=${encodeURIComponent(query)}&per_page=1&orientation=portrait&client_id=${unsplashKey}`
      );
      if (res.ok) {
        const data = await res.json() as { results?: Array<{ urls?: { small?: string } }> };
        const url = data.results?.[0]?.urls?.small;
        if (url) { imageCache.set(cacheKey, url); return url; }
      }
    } catch { /* fall through */ }
  }
  ...
  // Deterministic fallback — always works, no key required
...
function OutfitItemCard({
  ...
  useEffect(() => {
    ...
    fetchOutfitItemImage(item, gender)
      .then((url) => { if (mounted) setImageUrl(url); })
      .catch(() => { if (mounted) setImageUrl(fallback); })
      .finally(() => { if (mounted) setLoadingImage(false); });

    return () => { mounted = false; };
  }, [item.name, item.color, gender]); // primitives — stable deps
...
export default function DynamicOutfitCard({ outfit, onSaveToggle, onWoreRating, gender }: Props) {
